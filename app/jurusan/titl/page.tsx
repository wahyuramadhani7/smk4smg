...
const DEFAULT: Jurusan = {
  ...
  subtitle: 'Program Keahlian TITL · SMK Negeri 4 Semarang',
  profil_description:
    'Program keahlian Teknik Instalasi Tenaga Listrik mempersiapkan siswa untuk menguasai teknologi kelistrikan dan instalasi tenaga listrik secara profesional. Lulusan mampu merencanakan, memasang, dan memelihara instalasi listrik bangunan maupun industri sesuai standar PLN dan regulasi ketenagalistrikan nasional.',
  kompetensi: [
    'Memasang instalasi listrik bangunan gedung dan perumahan',
    'Mengoperasikan dan memelihara panel listrik & sistem distribusi',
    'Memasang instalasi motor listrik dan sistem kontrol',
    'Membaca dan membuat gambar instalasi kelistrikan',
    'Menerapkan K3 (Keselamatan & Kesehatan Kerja) di bidang kelistrikan',
  ],
  icon: '⚡',
  color: '#1d4ed8',
};

export default function JurusanTITL() {
  ...
  return (
    <>
      <style>{`
        ...
        .titl-title  { animation: fadeDown .8s .1s  ease both; }
        .titl-icon   { animation: scaleIn  .7s .15s ease both; }
        .titl-sub    { animation: fadeUp   .8s .3s  ease both; }
        .titl-bar    { animation: fadeUp   .8s .35s ease both; }
        .titl-footer { animation: fadeUp   .8s .6s  ease both; }
        ...
        .titl-kompetensi-item:last-child { border-bottom: none; }
        .titl-kompetensi-item:hover { color: #111827; }
      `}</style>
      ...
      <main style={{ minHeight: '100vh', background: '#ffffff', fontFamily: "'Plus Jakarta Sans', sans-serif", paddingBottom: 80 }}>
        ...
        <section style={{ position: 'relative', padding: '80px 24px 72px', textAlign: 'center', overflow: 'hidden', background: '#f9fafb', borderBottom: '1px solid #e5e7eb' }}>
          ...
          <div style={{ position: 'relative', zIndex: 1 }}>
            ...
            {/* eyebrow */}
            <div className="titl-title" style={{ display: 'inline-flex', alignItems: 'center', gap: 12, marginBottom: 20 }}>
            ...
            {/* icon */}
            <div className="titl-icon" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 20px', width: 80, height: 80, borderRadius: 24, background: '#eff6ff', border: '1.5px solid #bfdbfe', fontSize: '2.4rem' }}>
              {jurusan.icon}
            </div>

            {/* title */}
            <h1 className="titl-title" style={{ fontFamily: "'Plus Jakarta Sans', sans-serif", fontSize: 'clamp(1.8rem,5vw,3.2rem)', fontWeight: 800, lineHeight: 1.15, letterSpacing: '-0.01em', color: '#111827', margin: '0 0 16px' }}>
              {jurusan.nama_lengkap}
            </h1>

            {/* subtitle */}
            <p className="titl-sub" style={{ fontSize: 'clamp(.95rem,1.8vw,1.05rem)', color: '#6b7280', lineHeight: 1.8, maxWidth: 520, margin: '0 auto 24px', whiteSpace: 'pre-line', fontWeight: 500 }}>
              {jurusan.subtitle}
            </p>

            {/* accent bar */}
            <div className="titl-bar" style={{ width: 60, height: 4, borderRadius: 9999, background: 'linear-gradient(90deg, #1d4ed8, #f59e0b)', margin: '0 auto' }} />
          </div>
        </section>
        ...
        <div style={{ maxWidth: '960px', margin: '0 auto', padding: '56px 24px 0' }}>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: 24, marginBottom: 32 }}>
            ...
            {/* Card 1 — Profil */}
            <div className="titl-card" style={{ borderRadius: 16, overflow: 'hidden', border: '1px solid #e5e7eb', display: 'flex', flexDirection: 'column', background: '#ffffff', boxShadow: '0 4px 20px rgba(0,0,0,.05)', animation: 'scaleIn .7s .25s ease both' }}>
            ...
            {/* Card 2 — Kompetensi */}
            <div className="titl-card" style={{ borderRadius: 16, overflow: 'hidden', border: '1px solid #e5e7eb', display: 'flex', flexDirection: 'column', background: '#ffffff', boxShadow: '0 4px 20px rgba(0,0,0,.05)', animation: 'scaleIn .7s .4s ease both' }}>
              ...
              <div style={{ padding: 32, flex: 1, display: 'flex', flexDirection: 'column' }}>
                ...
                <ul style={{ listStyle: 'none', padding: 0, margin: 0, flex: 1 }}>
                  {jurusan.kompetensi.map((item, i) => (
                    <li key={i} className="titl-kompetensi-item">
          ...
          {/* Footer note */}
          <div className="titl-footer" style={{ textAlign: 'center', padding: '20px 32px', borderRadius: 12, background: '#f9fafb', border: '1px solid #e5e7eb', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 12 }}>
            ...
            <p style={{ fontSize: 13, color: '#6b7280', whiteSpace: 'pre-line', margin: 0, fontWeight: 500 }}>
              {'Butuh informasi lebih lanjut tentang jurusan Teknik Instalasi Tenaga Listrik?\nSilakan hubungi Waka Kurikulum SMK Negeri 4 Semarang'}
            </p>
          </div>
        </div>
