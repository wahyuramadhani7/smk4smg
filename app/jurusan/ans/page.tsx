...
const DEFAULT: Jurusan = {
  nama_lengkap: 'Animasi',
  subtitle: 'Program Keahlian Animasi · SMK Negeri 4 Semarang',
  profil_description:
    'Program keahlian Animasi mempersiapkan siswa untuk menguasai seni dan teknologi animasi secara profesional. Lulusan mampu merancang, memproduksi, dan menyelesaikan karya animasi 2D maupun 3D untuk berbagai kebutuhan industri kreatif, periklanan, game, film, dan konten digital sesuai standar industri nasional maupun internasional.',
  kompetensi: [
    'Membuat animasi 2D menggunakan Adobe Animate dan teknik frame-by-frame',
    'Memproduksi animasi 3D dengan software Blender atau sejenisnya',
    'Merancang karakter, storyboard, dan naskah cerita animasi',
    'Membuat motion graphic dan visual effect untuk konten digital',
    'Mengolah audio-visual dan rendering hasil produksi animasi',
  ],
  icon: '🎬',
  color: '#1d4ed8',
};
  ...
  return (
    <>
      <style>{`
        ...
        .anim-title  { animation: fadeDown .8s .1s  ease both; }
        .anim-icon   { animation: scaleIn  .7s .15s ease both; }
        .anim-sub    { animation: fadeUp   .8s .3s  ease both; }
        .anim-bar    { animation: fadeUp   .8s .35s ease both; }
        .anim-footer { animation: fadeUp   .8s .6s  ease both; }
        ...
        .anim-kompetensi-item:last-child { border-bottom: none; }
        .anim-kompetensi-item:hover { color: #111827; }
      `}</style>
      ...
      <main style={{ minHeight: '100vh', background: '#ffffff', fontFamily: "'Plus Jakarta Sans', sans-serif", paddingBottom: 80 }}>
        ...
        <section style={{ position: 'relative', padding: '80px 24px 72px', textAlign: 'center', overflow: 'hidden', background: '#f9fafb', borderBottom: '1px solid #e5e7eb' }}>
          ...
          <div style={{ position: 'relative', zIndex: 1 }}>
            ...
            {/* eyebrow */}
            <div className="anim-title" style={{ display: 'inline-flex', alignItems: 'center', gap: 12, marginBottom: 20 }}>
            ...
            {/* icon */}
            <div className="anim-icon" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 20px', width: 80, height: 80, borderRadius: 24, background: '#eff6ff', border: '1.5px solid #bfdbfe', fontSize: '2.4rem' }}>
              {jurusan.icon}
            </div>

            {/* title */}
            <h1 className="anim-title" style={{ fontFamily: "'Plus Jakarta Sans', sans-serif", fontSize: 'clamp(1.8rem,5vw,3.2rem)', fontWeight: 800, lineHeight: 1.15, letterSpacing: '-0.01em', color: '#111827', margin: '0 0 16px' }}>
              {jurusan.nama_lengkap}
            </h1>

            {/* subtitle */}
            <p className="anim-sub" style={{ fontSize: 'clamp(.95rem,1.8vw,1.05rem)', color: '#6b7280', lineHeight: 1.8, maxWidth: 520, margin: '0 auto 24px', whiteSpace: 'pre-line', fontWeight: 500 }}>
              {jurusan.subtitle}
            </p>

            {/* accent bar */}
            <div className="anim-bar" style={{ width: 60, height: 4, borderRadius: 9999, background: 'linear-gradient(90deg, #1d4ed8, #f59e0b)', margin: '0 auto' }} />
          </div>
        </section>
        ...
        <div style={{ maxWidth: '960px', margin: '0 auto', padding: '56px 24px 0' }}>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: 24, marginBottom: 32 }}>
            ...
            {/* Card 1 — Profil */}
            <div className="anim-card" style={{ borderRadius: 16, overflow: 'hidden', border: '1px solid #e5e7eb', display: 'flex', flexDirection: 'column', background: '#ffffff', boxShadow: '0 4px 20px rgba(0,0,0,.05)', animation: 'scaleIn .7s .25s ease both' }}>
            ...
            {/* Card 2 — Kompetensi */}
            <div className="anim-card" style={{ borderRadius: 16, overflow: 'hidden', border: '1px solid #e5e7eb', display: 'flex', flexDirection: 'column', background: '#ffffff', boxShadow: '0 4px 20px rgba(0,0,0,.05)', animation: 'scaleIn .7s .4s ease both' }}>
              ...
              <div style={{ padding: 32, flex: 1, display: 'flex', flexDirection: 'column' }}>
                ...
                <ul style={{ listStyle: 'none', padding: 0, margin: 0, flex: 1 }}>
                  {jurusan.kompetensi.map((item, i) => (
                    <li key={i} className="anim-kompetensi-item">
          ...
          {/* Footer note */}
          <div className="anim-footer" style={{ textAlign: 'center', padding: '20px 32px', borderRadius: 12, background: '#f9fafb', border: '1px solid #e5e7eb', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 12 }}>
            ...
            <p style={{ fontSize: 13, color: '#6b7280', whiteSpace: 'pre-line', margin: 0, fontWeight: 500 }}>
              {'Butuh informasi lebih lanjut tentang jurusan Animasi?\nSilakan hubungi Waka Kurikulum SMK Negeri 4 Semarang'}
            </p>
          </div>
        </div>
