  ...
  return (
    <>
      <style>{`
        ...
        .dpib-kompetensi-item:hover { color: #111827; }
      `}</style>
      ...
      <main style={{ minHeight: '100vh', background: '#ffffff', fontFamily: "'Plus Jakarta Sans', sans-serif", paddingBottom: 80 }}>
        ...
        <section style={{ position: 'relative', padding: '80px 24px 72px', textAlign: 'center', overflow: 'hidden', background: '#f9fafb', borderBottom: '1px solid #e5e7eb' }}>
          ...
          <div style={{ position: 'relative', zIndex: 1 }}>
            ...
            {/* icon */}
            <div className="dpib-icon" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 20px', width: 80, height: 80, borderRadius: 24, background: '#eff6ff', border: '1.5px solid #bfdbfe', fontSize: '2.4rem' }}>
              {jurusan.icon}
            </div>

            {/* title */}
            <h1 className="dpib-title" style={{ fontFamily: "'Plus Jakarta Sans', sans-serif", fontSize: 'clamp(1.8rem,5vw,3.2rem)', fontWeight: 800, lineHeight: 1.15, letterSpacing: '-0.01em', color: '#111827', margin: '0 0 16px' }}>
              {jurusan.nama_lengkap}
            </h1>

            {/* subtitle */}
            <p className="dpib-sub" style={{ fontSize: 'clamp(.95rem,1.8vw,1.05rem)', color: '#6b7280', lineHeight: 1.8, maxWidth: 520, margin: '0 auto 24px', whiteSpace: 'pre-line', fontWeight: 500 }}>
              {jurusan.subtitle}
            </p>

            {/* accent bar */}
            <div className="dpib-bar" style={{ width: 60, height: 4, borderRadius: 9999, background: 'linear-gradient(90deg, #1d4ed8, #f59e0b)', margin: '0 auto' }} />
          </div>
        </section>
        ...
        <div style={{ maxWidth: '960px', margin: '0 auto', padding: '56px 24px 0' }}>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: 24, marginBottom: 32 }}>
            ...
            {/* Card 1 — Profil */}
            <div className="dpib-card" style={{ borderRadius: 16, overflow: 'hidden', border: '1px solid #e5e7eb', display: 'flex', flexDirection: 'column', background: '#ffffff', boxShadow: '0 4px 20px rgba(0,0,0,.05)', animation: 'scaleIn .7s .25s ease both' }}>
            ...
            {/* Card 2 — Kompetensi */}
            <div className="dpib-card" style={{ borderRadius: 16, overflow: 'hidden', border: '1px solid #e5e7eb', display: 'flex', flexDirection: 'column', background: '#ffffff', boxShadow: '0 4px 20px rgba(0,0,0,.05)', animation: 'scaleIn .7s .4s ease both' }}>
          ...
          {/* Footer note */}
          <div className="dpib-footer" style={{ textAlign: 'center', padding: '20px 32px', borderRadius: 12, background: '#f9fafb', border: '1px solid #e5e7eb', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 12 }}>
